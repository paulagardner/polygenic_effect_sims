...
def run_sim(args: argparse.Namespace) -> fwdpy11.DiploidPopulation:
    ...
    demography = fwdpy11.discrete_demography.from_demes("pop_split.yaml", burnin=1)
    ...
    paramsdict = {
        ...
        "simlen": demography.metadata["total_simulation_length"],
    ...
    # since pop.N changes through here, evolvets is working fine.
    ...
    return (
        ...
        E_MEAN,
        E_SD,
    )  # if you don't return one of the variables: you'll get an error in write_treefile: 'AttributeError: 'NoneType' object has no attribute 'dump_tables_to_tskit'
...
def write_treefile(
    *,
    ...
    ts = pop.dump_tables_to_tskit(  # The actual model params
        ...
        demes_graph=demes.load("pop_split.yaml"),
        parameters={
            "seed": seed,
            "simplification_interval": 100,
            "meanE": E_MEAN,
    ...
    # g = demes.Graph.fromdict(ts.ts.metadata["demes_graph"])
    # assert g == graph

    # assert demes.Graph.fromdict(ts.ts.metadata["demes_graph"]) == graph
    # print()
    # print(ts.demes_graph)
